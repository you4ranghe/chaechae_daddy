...
export function AnalyticsHistory() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/agent/analytics?page=${page}`)
      .then((r) => r.json())
      .then((d) => {
        if (cancelled) return;
  ...
  return (
    <div className="mt-10">
      <h3 className="text-sm font-semibold text-gray-700">지난 분석 기록</h3>
              ...
              <div key={item.id} className="rounded-xl border border-gray-200 bg-white">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">{item.report.competitiveness.tier}</p>
                      <p className="text-xs text-gray-400">{formatDate(item.created_at)}</p>
                    </div>
                  </div>
                  <span className="text-xs text-gray-400">{expanded ? "접기" : "보기"}</span>
                </button>
                {expanded && (
