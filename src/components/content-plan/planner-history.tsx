...
export function PlannerHistory() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/agent/content-plan?page=${page}`)
      .then((r) => r.json())
      .then((d) => {
        if (cancelled) return;
  ...
  return (
    <div className="mt-10">
      <h3 className="text-sm font-semibold text-gray-700">지난 콘텐츠 플랜</h3>
      <p className="mt-0.5 text-xs text-gray-400">최근 생성한 플랜부터 표시돼요</p>
      ...
      {data && data.items.length > 0 && (
        <div className="mt-3 space-y-2">
          {data.items.map((item) => {
            ...
            const dayCount = item.plan?.weeklyPlan?.length ?? 0;
            return (
              <div key={item.id} className="rounded-xl border border-gray-200 bg-white">
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-gray-900">
                      <span className="text-indigo-600">{item.keywords}</span>
                    </p>
                    <p className="mt-0.5 text-xs text-gray-400">
                      {formatDate(item.created_at)} · {dayCount}일 플랜
                    </p>
                  </div>
                ...
                {expanded && (
                  <div className="border-t border-gray-100 px-4 py-4">
                    <WeeklyPlanView result={item.plan} />
                  </div>
                )}
              </div>
            );
