...
export default function ForgotPasswordModal({
  ...
  // 코드 단계 카운트다운
  useEffect(() => {
    if (!open || step !== "code" || secondsLeft <= 0) return;
    const t = setInterval(() => {
  ...
  async function requestCode(targetEmail: string): Promise<boolean> {
    ...
    if (authError) {
      ...
      if (status === 429 || msg.includes("rate") || msg.includes("too many")) {
        setError(
          "이메일 발송 횟수 제한에 걸렸어요. 1시간 정도 후에 다시 시도해 주세요."
        );
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    try {
      ...
      if (uErr) {
        ...
        if (msg.includes("different from the old") || msg.includes("same") || msg.includes("same_password")) {
          setError("새 비밀번호는 기존 비밀번호와 달라야 해요.");
        } else if (msg.includes("weak") || msg.includes("at least") || msg.includes("characters")) {
          setError(`비밀번호 조건을 만족하지 않아요. (${uErr.message})`);
        } else if (msg.includes("session") || msg.includes("expired") || msg.includes("token")) {
          setError("인증 세션이 만료됐어요. 처음부터 다시 시도해 주세요.");
        } else {
          setError(`변경 실패: ${uErr.message}`);
        }
        return;
      }
      // 변경 후 새 로그인 유도를 위해 세션 정리
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-sm rounded-3xl border border-gray-100 bg-white p-6 shadow-2xl sm:p-7"
      ...
      >
        ...
        <h2 id="forgot-password-title" className="pr-8 text-xl font-bold text-gray-900">
          {heading}
        </h2>
        ...
        {step === "code" && (
          <form onSubmit={handleVerifyCode} className="mt-5 space-y-4">
            <CodeField value={code} onChange={setCode} secondsLeft={secondsLeft} />
            <PrimaryButton
              loading={loading}
              label="인증하기"
              loadingLabel="확인 중…"
              disabled={secondsLeft <= 0 || code.length < 6 || code.length > 8}
            />
        ...
        {step === "done" && (
          <div className="mt-5 space-y-4">
            <div className="flex items-center gap-3 rounded-xl bg-emerald-50 px-3 py-3 text-[13px] text-emerald-700 ring-1 ring-inset ring-emerald-100">
              ...
              <span>비밀번호가 안전하게 변경됐어요. 새 비밀번호로 로그인해 주세요.</span>
            </div>
            <button
              type="button"
              onClick={onClose}
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <label htmlFor="fp-code" className="block text-xs font-semibold text-gray-700">
          인증번호 <span className="font-medium text-gray-400">(이메일로 받은 숫자)</span>
        </label>
        <span
      ...
      >
        ...
        <input
          ...
          className="flex-1 bg-transparent py-2.5 pr-3.5 text-sm tracking-[0.35em] text-gray-900 placeholder:tracking-[0.35em] placeholder:text-gray-300 focus:outline-none disabled:cursor-not-allowed"
        />
      </div>
